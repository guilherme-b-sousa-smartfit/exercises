...
function Exercise({ row: r, index, selected, toggle }: { row: ComparisonRow; index: MediaIndex; selected: Set<string>; toggle: (key: string) => void }) {
  ...
  return <article className="card exercise"><div className="corpo">
    ...
    {asset && !asset.preview && kind !== 'image' && <small className="dica">Capa do produto; abra o site para assistir.</small>}
    ...
    <div className="select-media">{kinds.map(k => <label key={k}><input type="checkbox" disabled={!r[k].id || r[k].status === 'Não encontrado'} checked={selected.has(keyFor(k, r[k].id))} onChange={() => toggle(keyFor(k, r[k].id))} />{labels[k]}{r[k].status === 'Revisar' ? ' · revisar' : ''}</label>)}</div>
    <details><summary>Por que este candidato?</summary><p className="comparison-reason">{r.reason}</p><p className="dica">O índice é heurístico, não uma probabilidade. A prévia permite conferir execução, equipamento e variação.</p><a className="link" href={r.sourceUrl} target="_blank" rel="noreferrer">Registro original ↗</a></details>
  </div></article>;
}
...
export const App = () => {
  ...
  return <main className="app">
    <header className="cabecalho"><h1>Smart Fit × Gym Visual</h1><p>Compare a execução atual com as prévias do catálogo e monte uma estimativa de compra.</p></header>
    ...
    <section className="budget-panel" aria-label="Simulador de custos"><h2>Simular compra <small>USD · mídias únicas</small></h2>
      ...
      <details className="pricing"><summary>Preços e regras do orçamento</summary><p>Valores informados: US$ 0,90/GIF após 10 itens e US$ 6/vídeo após 5. Interpretamos “após” como 11 e 6, respectivamente. O site apresenta mínimos divergentes entre banner e produto. Ajuste conforme a cotação; não há compra automática.</p><div className="linha">{(['gif','video'] as const).map(k => <div key={k}><label>{labels[k]} · USD/unidade <input type="number" min="0" step="0.01" value={pricing[k === 'gif' ? 'gifCents' : 'videoCents']/100} onChange={e => setPricing(p => ({...p,[k === 'gif' ? 'gifCents' : 'videoCents']: Math.max(0, Math.round(Number(e.target.value)*100))}))} /></label><label>Mínimo de {labels[k]}s <input type="number" min="1" step="1" value={pricing[k === 'gif' ? 'gifMin' : 'videoMin']} onChange={e => setPricing(p => ({...p,[k === 'gif' ? 'gifMin' : 'videoMin']: Math.max(1,Math.floor(Number(e.target.value)))}))} /></label></div>)}<label>Imagem · USD/unidade (opcional)<input type="number" min="0" step="0.01" placeholder="Não informado" value={pricing.imageCents == null ? '' : pricing.imageCents/100} onChange={e => setPricing(p => ({...p,imageCents:e.target.value === '' ? null : Math.max(0,Math.round(Number(e.target.value)*100))}))} /></label></div><p className="dica">Abaixo do mínimo, só usamos preço avulso verificado no produto; caso contrário fica pendente. Desconto calculado por formato, sem impostos, câmbio ou descontos de pacotes. Prévia pública não é o arquivo adquirido.</p></details>
    </section>
